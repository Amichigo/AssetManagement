...
const BASE_API_URL = 'http://localhost:5000/api/SoldAsset';

@Component({
    templateUrl: './sold-assets.component.html',
    styleUrls: ['./sold-assets.component.css'],
    animations: [appModuleAnimation()]
})
export class SoldAssetsComponent extends AppComponentBase implements OnInit {
            ...
            case 1:
                switch (comparisonMode) {
                    case 0:
                        switch (valueType) {
                            case 0:
                                this.loadTotalSoldAssetsChartInOriginalValue();
                                break;

                            case 1:
                                this.loadTotalSoldAssetsChartInPercentage();
                                break;
                        }
                        break;

                    case 1:
                        this.loadTotalSoldAssetsChartWithComparison();
                        break;
                }
                break;
        }
    ...
    changeSection2(payload) {
        ...
        switch (activeTabIndex) {
            ...
            case 1:
                switch (valueType) {
                    case 0:
                        this.loadSoldAssetsByTypesInOriginalValue();
                        break;

                    case 1:
                        this.loadSoldAssetsByTypesInPercentage();
                        break;
                }
                break;
        }
    ...
    loadSoldAssetsByTypesInPercentage() {
        let labels = [];
        let processedData = [];
    ...
    loadSoldAssetsByTypesInOriginalValue() {
        let labels = [];
        let processedData = [];
        let abc = [];
    ...
    loadGeneralStatisticsData(data) {
        this.generalStatisticsData = [
            ...
            {
                title: 'Tổng số tài sản đã thanh lý',
                startingDate: this.startingDate,
                endingDate: this.endingDate,
    ...
    loadTotalSoldAssetsChartInOriginalValue() {
        ...
        this.section1Labels = labels;
        this.section1Data = [{ data: temp, label: 'Số lượng tài sản đã thanh lý (Tài sản)' }];
        this.section1YAxeLabel = 'Số lượng tài sản đã thanh lý (Tài sản)';
    }

    loadTotalSoldAssetsChartInPercentage() {
        let labels = [];
        let processedData = [];
        ...
        this.section1Labels = labels;
        this.section1YAxeLabel = 'Tỉ lệ phần trăm so với tổng số lượng tài sản đã thanh lý (%)';
        this.section1Data = [{ data: temp, label: 'Tỉ lệ phần trăm so với tổng số lượng tài sản đã thanh lý (%)' }];
    }

    loadTotalSoldAssetsChartWithComparison() {
        let processedData1 = [];
        let processedData2 = [];
        let labels = [];
